...
func TestNetworkEndpoint_GetAddress(t *testing.T) {
	neUnix := NetworkEndpoint{
		Family:  AddressFamilyUnix,
		Address: "/var/run/test/test.sock",
	}
	aUnix := neUnix.GetAddress()
	...
	neIP := NetworkEndpoint{
		Family:  AddressFamilyTCP,
...
func TestNetworkEndpoint_ValidateAddress(t *testing.T) {
	testCases := []struct {
		name  string
		ne    NetworkEndpoint
		valid bool
	}{
		{"Unix OK", NetworkEndpoint{Family: AddressFamilyUnix, Address: "/absolute/path"}, true},
		{"IP OK", NetworkEndpoint{Address: "12.3.4.5", Port: 76}, true},
		{"Unix not absolute", NetworkEndpoint{Family: AddressFamilyUnix, Address: "./socket"}, false},
		{"IP invalid", NetworkEndpoint{Address: "260.3.4.5", Port: 76}, false},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := tc.ne.ValidateAddress()
			if tc.valid && err != nil {
